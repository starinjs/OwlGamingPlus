...
addEventHandler( 'onResourceStart', resourceRoot, function()
	...
	end, {}, exports.mysql:getConn(), "SELECT m.*, f.id AS existed FROM mdc_groups m LEFT JOIN factions f ON m.faction_id=f.id" )
end)

-- this exported function is also used by /deletefaction in faction system.
function cleanUpMdcUsersAndGroups( faction_id )
	dbExec( exports.mysql:getConn(), "DELETE FROM mdc_groups WHERE faction_id=? ", faction_id )
	dbExec( exports.mysql:getConn(), "DELETE FROM mdc_users WHERE organization=? ", faction_id )
end
...
addCommandHandler( 'convertmdcaccounts', function(p, c)
	if exports.integration:isPlayerScripter( p ) then
		local qh = dbQuery( exports.mysql:getConn(), "SELECT * FROM mdc_users" )
		local res, nums, id = dbPoll( qh, 10000 )
		if res and nums > 0 then
			for i, row in pairs( res ) do
				local id_ = getIdFromOrg( row.organization )
				if id_ then
					dbExec( exports.mysql:getConn(), "UPDATE mdc_users SET organization=? WHERE id=?", id_, row.id )
	...
	if exports.integration:isPlayerScripter( p ) then
		...
		if res and nums > 0 then
			...
			for charid, count in pairs( users ) do
				if count > 1 then
					dbExec( exports.mysql:getConn(), "DELETE FROM mdc_users WHERE `level`=1 AND charid=?", charid )
